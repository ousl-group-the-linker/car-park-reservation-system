            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                    <a href="{{ route('branches-management.admin-management', ['branch' => $branch->id]) }}"
                        class="text-decoration-none text-dark">Admin Management</a>

                </li>
                <li class="breadcrumb-item active" aria-current="page">Admin #{{ $admin->id }}</li>
            </ol>
        </nav>
    </div>
@endsection
...
@section('main-body')
    <div class="container">
        <div class="card" style="max-width: 680px">
            ...
            <form
                action="{{ route('branches-management.admin-management.edit', ['branch' => $branch->id, 'admin' => $admin->id]) }}"
                method="post">
                @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @if ($errors->has('role')) is-invalid @endif" aria-label="City"
                            name="role" required @if (!Auth::user()->isSuperAdminAccount()) disabled @endif>
                            <option selected disabled>Choose one</option>
                    ...
                    @if ((Auth::user()->id !== $admin->id && Auth::user()->isManagerAccount()) || Auth::user()->isSuperAdminAccount())
                        <div class="mb-3">
                            ...
                            <select class="form-select @if ($errors->has('is_activated')) is-invalid @endif"
                                ...
                                <option value="1" @if ((int) old('is_activated', $admin->is_activated) === 1) selected @endif>Activated
                                </option>
                                <option value="0" @if ((int) old('is_activated', $admin->is_activated) === 0) selected @endif>Deactivated
                                </option>
                ...
                <div class="card-footer d-flex justify-content-between">
                    ...
                    <div class="d-flex flex-row">
                        ...
                        <a href="{{ route('branches-management.admin-management.edit', ['branch' => $branch->id, 'admin' => $admin->id]) }}"
