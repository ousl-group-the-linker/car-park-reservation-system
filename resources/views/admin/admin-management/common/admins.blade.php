    ...
    <div class="container p-0">
        <div class="row mb-4 p-0 m-0">
            <div class="col-12 d-flex p-0 m-0" style="max-width: 780px">
                <a href="{{ $new_endpoint }}" class="btn btn-primary">
        ...
        <div class="p-0" style="max-width: 780px">
            <div class="card">
                ...
                <form action="{{ $source_endpoint }}" method="get">
                    ...
                    <div class="card-footer d-flex justify-content-end">
                        <a href="{{ $source_endpoint }}" class="btn btn-light me-2"><i
                ...
                @foreach ($admin_accounts as $admin_account)
                    <div class="row border rounded bg-light py-3 px-4 mx-0  mb-2">
                        ...
                        <div class="col-12 col-sm-6 col-xl-3 mb-2 mb-xl-0 p-0 d-flex flex-column align-items-start">
                            ...
                            @if ($admin_account->isManagerAccount())
                                <p class="text-wrap m-0">{{ $admin_account->ManageBranch->name ?? 'No Branch' }}</p>
                            @elseif($admin_account->isCounterAccount())
                                <p class="text-wrap m-0">{{ $admin_account->WorkForBranch->name ?? 'No Branch' }}</p>
                            @endif
                        </div>
                        ...
                        <div
                            ...
                            @can('view', $admin_account)
                                <a target="_blank"
                                    href="{{ route('super-admin.admin-management.view', ['admin' => $admin_account->id]) }}"
                                    class="btn btn-light me-2"><i class="bi bi-box-arrow-up-right me-1"></i>View</a>
                            @endcanany

                            @can('update', $admin_account)
                                <a target="_blank"
                                    href="{{ route('super-admin.admin-management.edit', ['admin' => $admin_account->id]) }}"
